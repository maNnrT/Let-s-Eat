...
function HeaderItem({ title, to, display,onClick }: Props) {
  ...
  return (
    <NavLink
      ...
      className={(navData) =>
        cx(
          'lg_desktop:text-[1.8rem] text-[1.6rem] leading-[3rem] tracking-[0.03em] p-[1.2rem] font-medium hover:text-secondary hover:font-bold hover:tracking-[0em] hover:duration-200',
          { active: navData.isActive },
        )
      }
      title={title}
